...
import com.chauncy.jt808.util.BCD8421Operater;
...
import com.chauncy.jt808.vo.req.TerminalRegisterMsg.TerminalRegInfo;
...
public class MsgDecoder {
    ...
    public PackageData bytesPackageData(byte[] data) {
    ...
    private MsgHeader parseMsgHeaderFromBytes(byte[] data) {
        ...
        // msgHeader.setTerminalPhone(this.bcd8421Operater.bcdString(tmp));
    ...
    private String parseStringFromBytes(byte[] data, int startIndex, int lenth, String defaultVal) {
        try {
            ...
            return new String(tmp, Command.STRING_CHARSET);
        } catch (Exception e) {
    ...
    private String parseBcdStringFromBytes(byte[] data, int startIndex, int lenth, String defaultVal) {
        try {
            byte[] tmp = new byte[lenth];
            System.arraycopy(data, startIndex, tmp, 0, lenth);
            return this.bcd8421Operater.bcdString(tmp);
        } catch (Exception e) {
    ...
    public LocationInfoUploadMsg toLocationInfoUploadMsg(PackageData packageData) {
        ...
        ret.setTime(new Date(Long.parseLong(time)));
        ByteBuf buff = Unpooled.copiedBuffer(data);
        buff.readBytes(28);

        //附加信息解析
        while (buff.isReadable()) {
            byte additionalId = buff.readByte();
            byte additionalLength = buff.readByte();
            switch (additionalId) {
                case 0x01:
                    //里程
                    ret.setMileage(buff.readInt() & 0xffffffffL);
                    break;
                case 0x02:
                    //油量
                    ret.setFuel(buff.readShort() & 0xffff);
                    break;
                case 0x03:
                    //行驶记录仪速度
                    ret.setRecorderSpeed(buff.readShort() & 0xffff);
                    break;
            }
        }
        return ret;
